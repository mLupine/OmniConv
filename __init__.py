...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    async def send_prompt(call: ServiceCall) -> ServiceResponse:
        ...
        if filenames := call.data.get(CONF_FILENAMES):
            ...
            content.extend(await async_prepare_files_for_prompt(hass, [Path(filename) for filename in filenames]))
        ...
        model_args = {
            "model": model,
            "input": messages,
            "max_output_tokens": conversation_subentry.data.get(CONF_MAX_TOKENS, RECOMMENDED_MAX_TOKENS),
        ...
        if model.startswith("o"):
            model_args["reasoning"] = {
                "effort": conversation_subentry.data.get(CONF_REASONING_EFFORT, RECOMMENDED_REASONING_EFFORT)
            }
...
async def async_setup_entry(hass: HomeAssistant, entry: OmniConvConfigEntry) -> bool:
    ...
    client = openai.AsyncOpenAI(
        ...
        default_query=({"api-version": entry.data[CONF_API_VERSION]} if entry.data.get(CONF_API_VERSION) else None),
        http_client=get_async_client(hass),
    )

    _ = await hass.async_add_executor_job(client.platform_headers)

    if not entry.data.get(CONF_SKIP_AUTH):
        try:
...
async def async_migrate_integration(hass: HomeAssistant) -> None:
    ...
    for entry in entries:
        ...
        if entry.data[CONF_API_KEY] not in api_keys_entries:
            use_existing = True
            all_disabled = all(
                e.disabled_by is not None for e in entries if e.data[CONF_API_KEY] == entry.data[CONF_API_KEY]
            )
...
def _add_ai_task_subentry(hass: HomeAssistant, entry: OmniConvConfigEntry) -> None:
