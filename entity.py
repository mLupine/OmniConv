    ...
    async for event in stream:
        ...
        if isinstance(event, ResponseOutputItemAddedEvent):
            if isinstance(event.item, ResponseFunctionToolCall):
                ...
                or (
                    isinstance(event.item, ResponseReasoningItem) and last_summary_index is not None
...
class OmniConvBaseLLMEntity(Entity):
    ...
    async def _async_handle_chat_log(
        ...
        if options.get(CONF_ENABLE_FUNCTIONS):
            _registry = tools.create_registry(self.hass, options.get(CONF_FUNCTIONS, []))
            if chat_log.llm_api:
                chat_log.llm_api.tools.extend(_registry.tools)
            else:
                llm_context = llm.LLMContext(
                    platform=DOMAIN, context=None, language=None, assistant=None, device_id=None
                )
                chat_log.llm_api = llm.APIInstance(
                    _InlineAPI(hass=self.hass, id=DOMAIN, name=DOMAIN),
                    "",
                    llm_context,
                    list(_registry.tools),
                )
        ...
        if model_args["model"].startswith(("o", "gpt-5")):
            model_args["reasoning"] = {
                "effort": options.get(CONF_REASONING_EFFORT, RECOMMENDED_REASONING_EFFORT),
                "summary": "auto",
            }
        ...
        if chat_log.llm_api:
            openai_tools = [_format_tool(tool, chat_log.llm_api.custom_serializer) for tool in chat_log.llm_api.tools]
        ...
        use_tools = options.get(CONF_FUNCTION_STYLE, "tools") == "tools"
        if openai_tools:
            if use_tools:
                model_args["tools"] = openai_tools
            else:
        ...
        max_iterations = options.get(CONF_MAX_FUNCTION_CALLS, MAX_TOOL_ITERATIONS)
